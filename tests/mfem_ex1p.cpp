...
int main(int argc, char *argv[])
{
   ...
   const char *mesh_file = (std::string(SERAC_REPO_DIR) + "/data/meshes/star.mesh").c_str();
   int order = 2;
...
#ifdef GENERATE_BALLMESH
     auto pmesh_serac = serac::buildBallMesh(10000);
     pmesh_serac->ReorientTetMesh();
     std::ofstream mesh_out("ballmesh.reorder.mesh");
     pmesh_serac->Print(mesh_out);
     Mesh m("ballmesh.reorder.mesh");
     ParMesh pmesh(*pmesh_serac);
   ...
   x = 0.0;
   x.SetSubVector(ess_tdof_list, 1.0);
   ...
   else if (amgx_lib && strcmp(amgx_json_file,"") == 0)
   {
      ...
      // const serac::IterativeSolverParameters linear_params = {.rel_tol     = 1.0e-12,
      // 							      .abs_tol     = 1.0e-12,
      // 							      .print_level = 1,
      // 							      .max_iter    = 2000,
      // 							      .lin_solver  = serac::LinearSolver::CG,
      // 							      .prec        = serac::AMGXPrec{}};
      // serac::EquationSolver serac_solver(MPI_COMM_WORLD, linear_params);
      // serac_solver.SetOperator(*A);
      // serac_solver.Mult(B,X_serac);

      // Vector diff = X - X_serac;
      // double Xdot = X*X;
      // double X_seracdot = X_serac * X_serac;
      // std::cout <<  diff.Norml2() << " " << Xdot << " " << X_seracdot << std::endl;
   ...
   std::cout << "Calculate norm:" << x.Norml2() << std::endl;
